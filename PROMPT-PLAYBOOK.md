...
- 14.07. **quality_lock-Falle bei „seen from behind"-Szenen (Cover-Runde Gift-Listings):** `quality_lock:true` bei Ganzkörper-Rückenansicht mit Panorama → Seedream montiert ein riesiges Geister-Gesicht in den Himmel (Poster-Doppelbelichtung, 2/3 Ausschuss). **Fix: `quality_lock:false` + Präfix `Cinematic film still, photoreal:` + „small lone figure"** → 3/3 sauber. Threshold-Shots sind also LOCATION-Regeln, nicht Charakter-Regeln. Marco-approved aus der Runde: threshold2-1 (Mantel im Wind, brennende Stadt), villain-2 (Silhouette-Formel erneut bestätigt: gekrönte Gestalt auf Burgruine), gate-2 (Fantasy-Anker + Runen; Runen tendieren zu grellem Türkis — ggf. „faint golden runes" testen), grave-2 (Story-Objekt-Trauer: Schwert + zerrissener Umhang + Rabe am Steingrab, Morgennebel — NEUES Pattern „Memorial-Still" für Fallen-Hero-Aufträge).
