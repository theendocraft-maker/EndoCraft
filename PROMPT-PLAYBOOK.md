...
- 21.07. **Engine-Learning (Claude, bestätigt an 12 Bildern):** Der Live-`/api/image`-Endpoint erzwingt per Default (`quality_lock:AN`) einen Character-Prefix → dominantes Gesicht + Hände, selbst bei reinen Location-Prompts, in **Portrait UND Landscape** (Aspect hilft NICHT). Fix bestätigt: Body-Feld **`quality_lock:false`** mitsenden → saubere, figurenfreie Atmosphäre im Haus-Stil. Für Location/Ambience-Batches also IMMER `quality_lock:false`. (Zugang: POST mit Header `Origin: https://endocraft.app`, sonst 403.)
